              ...
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{ Str::limit($article->sub_title, 20) }}</td>
                    <td>{{ Str::limit($article->description, 20) }}</td>
                    <td>{{ $article->date }}</td>
                    <td>
                      <img src="{{ asset('article/'.$article->images)}}" alt="" width="50" height="50">
                    </td>
                    {{-- <td>
                      <td><a onclick="statusUpdate(event)" href="{{ route('psychologists.status', $article->id) }}" class="btn btn-sm btn-{{ $article->status == '1' ? 'success' : 'danger' }}">{{ $article->status }}</a></td>
                    </td> --}}
                    <td class="d-flex justify-center gap-3">
                        <a href="{{ route('article.edit', $article->id) }}"><i class="fa-solid fa-pen-to-square"></i></a>
