  ...
  <div class="main-content">
    <div class="main-content-container">
          ...
          <div class="section-title mt-4 d-flex justify-content-between">
            <h2>Psychologists</h2>
            <a href="{{ route('psychologists.create') }}" class="btn btn-sm btn-primary">Create</a>
          </div>
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th scope="col">Card Title 1</th>
                    <th scope="col">Card Description 1</th>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>{{ Str::limit($psychologists->title, 20) }}</td>
                    <td>{{ Str::limit($psychologists->short_description, 20) }}</td>
                    <td>{{ Str::limit($psychologists->card_title_1, 20) }}</td>
                    <td>{{ Str::limit($psychologists->card_desc_1, 20) }}</td>
                    <td>{{ $psychologists->btn_text }}</td>
                    <td><a onclick="statusUpdate(event)" href="{{ route('psychologists.status', $psychologists->id) }}" class="btn btn-sm btn-{{ $psychologists->status == '1' ? 'success' : 'danger' }}">{{ $psychologists->status }}</a></td>
                    <td class="d-flex justify-center gap-3">
                        <a href="{{ route('psychologists.edit', $psychologists->id) }}"><i class="fa-solid fa-pen-to-square"></i></a>
                        {{-- <a onclick="deleteData(event)" href="{{ route('article.delete', $article->id) }}"><i class="fa-solid fa-trash"></i></a> --}}
                    </td>
                  </tr>                     
      ...
      function statusUpdate(e){
        ...
        .then((willCancel) => {
          if (willCancel) {
            ...
            swal("Your imaginary status is not changed!");
          }
        });
      }
    </script>
